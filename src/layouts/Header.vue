        ...
        <div class="wrapper" :class="'page_'+$route.name">
            <div class="header-block">
                ...
                <div class="header-act">
                    ...
                    <div v-if="!loading" @mouseenter="openmodal=true" @mouseleave="openmodal=false" class="login">
                        <span v-if="user">{{user?.first_name}} {{user?.sur_name}}</span>
                        <span @click="login" v-else>{{$t('log')}}</span>
                        <img @click="user?'':login" src="@/assets/icons/human.svg" alt="">
                        <div v-if="user?.first_name && openmodal" class="openmodal">
                            <router-link :to="'/'+this.$i18n.local+'/profile'">{{$t('myProfile')}}</router-link>
                            <router-link :to="'/'+this.$i18n.locale+'/applications'">{{$t('myAppl')}}</router-link>
                            <button @click="logout">{{$t('logout')}}</button>
                        </div>
                    </div>
                    <div @mouseenter="changeLang" @mouseleave="changeLang" class="header-langs">
                        <Icons :icon="activeLang.val" size="custom" width="24" height="16"/>
                        <p>{{activeLang.name}}</p> 
                        <div v-if="open" class="langs-block">
                            <div class="langs-item" v-for="el,index in langs" :key="index"><span class="tick"><Icons v-show="el.val == activeLang.val" icon="tick" size="custom" width="20" height="20"/></span><span @click="switchLang(el)" :class="{active:el.val == activeLang.val}"><Icons :icon="el.val" size="custom" width="24" height="16"/>{{el.name}}</span></div>
                        </div>
                    </div>
            ...
            <div class="header-elem-mobile" :class="{menu_mobile: menu, closed:close}">
                <div class="nav_block">
                    <div class="nav_menu">
                        <h5>{{$t('menu')}}</h5>
                        <Icons @click="menu = false,close=true" icon="searchClose" color="#7B8189" size="middle"/>
                    </div>
                    <div class="nav-items">
                        <h6>{{$t('lang')}}</h6>
                        <div class="nav-item">
                            <div :class="{active:el.val == activeLang.val}" class="nav-lang" v-for="el,index in langs" :key="index" @click="switchLang(el)"><Icons :icon="el.val" size="custom" width="24" height="16"/>{{el.name}}</div>
                        </div>
                    </div>
                    <div class="nav-items">
                         <div class="w-100 d-f gap-24 mt-64 jc-sb flex-wrap">
                            ...
                            <div class="fontSize w-50">
                                ...
                                <div class="font-mobile">
                                    ...
                                    >
                                        <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M22.1652 21.6769L18.284 10.8179L14.4028 21.6769H22.1652ZM20.2719 5.8492L28.9621 30.1508C28.9874 30.2713 29 30.3556 29 30.4038C29 30.8013 28.634 31 27.9019 31H26.8985C26.0149 31 25.4469 30.7169 25.1945 30.1508L23.0929 24.2787H13.475L11.3735 30.1508C11.1211 30.7169 10.5594 31 9.68847 31H8.11704C7.37235 31 7 30.8013 7 30.4038C7 30.3556 7.01262 30.2713 7.03787 30.1508L15.7281 5.8492C15.829 5.62034 16.0057 5.42159 16.2582 5.25295C16.5232 5.08432 16.8388 5 17.2048 5H18.8141C19.218 5 19.5273 5.06625 19.7418 5.19875C19.9816 5.35534 20.1583 5.57216 20.2719 5.8492Z" fill="currentColor"/>
                                        </svg>
                                    </div>
                                    <span>{{$t('bigText')}}</span>
                                </div>
...
export default {
  ...
  methods:{
    async login(){
        await window.open('https://sso.egov.uz/sso/oauth/Authorization.do?response_type=one_code&client_id=gidlar_reestri&redirect_uri=http://test.gidlar.uz/uz%2Fapplications&scope=gidlar_reestri&state=IDPW',"_self")
    },
    async logout(){
  ...
  mounted(){
    ...
    if(window.sessionStorage.colorScheme && window.sessionStorage.colorScheme !== '' && window.sessionStorage.fontSize && window.sessionStorage.fontSize !== '') {
      this.colorScheme = window.sessionStorage.colorScheme
      this.fontSize = window.sessionStorage.fontSize
    }
    if(window.sessionStorage.colorScheme && window.sessionStorage.colorScheme !== '' && window.sessionStorage.fontSize && window.sessionStorage.fontSize !== '') {
        document.getElementsByTagName( 'html' )[0].className = (window.sessionStorage.colorScheme + ' ' + window.sessionStorage.fontSize)
    } 
